...
export const PerfilAdmin = () => {
  ...
  const borrarPerfil = async (codigoPerfil: string) => {
    ...
    if (typeof resultado.eliminado === "undefined") {
      MensajeToastify(
        "error",
        "No se puede eliminar el perfil. Es posible que esté relacionado con usuarios",
        6000
      );
    } else {
              ...
              <tbody>
                {arregloPerfiles.map((perfilin, indice) => (
                  <tr key={indice}>
                    ...
                    <td>
                      {perfilin.estadoPerfil === 1 ? "Activo" : "Inactivo"}
                    </td>
                    <td>
                      <a
                        href="/#"
                      ...
                      {" "}
                      <Link to={"/dashboard/updateprofile/" + perfilin._id}>
                        <i
